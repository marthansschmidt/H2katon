...
export default function MagicRings({
  ...
  return <div ref={mountRef} className="magic-rings-container" style={{
    ...(blur > 0 ? { filter: `blur(${blur}px)` } : {}),
    left: positionX === 'left' ? 0 : positionX === 'right' ? 'auto' : '50%',
    right: positionX === 'right' ? 0 : 'auto',
    top: positionY === 'top' ? 0 : positionY === 'bottom' ? 'auto' : '50%',
    bottom: positionY === 'bottom' ? 0 : 'auto',
    transform: positionX === 'center' && positionY === 'center' ? 'translate(-50%, -50%)' : 
               positionX === 'center' ? 'translateX(-50%)' : 
               positionY === 'center' ? 'translateY(-50%)' : 'none'
  }} />;
}
